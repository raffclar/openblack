...
struct WaveHeader
{
	...
	char chunkId[4];   // "RIFF"
	int32_t chunkSize; // Size of the wav portion of the file, which follows the first 8 bytes. File size - 8
	char format[4];    // "WAVE"
};
...
struct FormatChunk
{
	...
	int32_t sampleRate;
	int32_t byteRate;         // Number of bytes per second. sample_rate * num_channels * Bytes Per Sample
	uint16_t sampleAlignment; // num_channels * Bytes Per Sample
	uint16_t bitDepth;        // Number of bits per sample
};

struct FormatChunkVariant
{
	// Variant
	uint16_t cbSize;
...
struct RiffChunk
{
	char format[4];           // Contains "data"
	int32_t dataBytes;        // Number of bytes in data. Number of samples * num_channels * sample byte size
};
}

const char* chunkIdMagic = "RIFF";
const char* waveHeaderFormatMagic = "WAVE";
const char* formatChunkMagic = "fmt ";
const char* riffChunkMagic = "data";
