...
void L3DMesh::Load(const l3d::L3DFile& l3d)
{
	...
	for (size_t i = 0; i < _subMeshes.size(); ++i)
	{
		...
		if (_subMeshes[i]->GetFlags().isPhysics)
		{
			auto& verticesSpan = l3d.GetVertexSpan(i);
			auto physicsMesh = new btConvexHullShape(reinterpret_cast<const btScalar*>(verticesSpan.data()),
			                                         verticesSpan.size(), sizeof(verticesSpan[0]));
